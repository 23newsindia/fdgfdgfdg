...
class SecurityWAF {
    ...
    public function __construct() {
        ...
        $this->request_limit = (int)get_option('security_waf_request_limit', 1000);
        $this->blacklist_threshold = (int)get_option('security_waf_blacklist_threshold', 20);
    ...
    public function waf_check() {
        ...
        if ($this->is_rate_limited($ip)) {
            ...
            // FIX: Removed block_request to avoid locking out legit users
    ...
    private function is_legitimate_browser($user_agent) {
        ...
        foreach ($legitimate_patterns as $pattern) {
            if (preg_match($pattern, $user_agent) && strpos($user_agent, 'Mozilla') !== false) {
                return true;
            }
        }
        return false;
    ...
    private function is_rate_limited($ip) {
        ...
        if ($this->is_woocommerce_ajax_request() || $this->is_woocommerce_admin_request() || $this->is_wordpress_core_request()) {
            return false; // FIX: Skip rate limit for WooCommerce & WP core
        }
        if (in_array($ip, array('103.251.55.45', '103.170.146.58'))) {
            return false;
        }
    ...
    private function log_violation($ip, $type) {
        ...
        try {
            ...
            // FIX: Removed automatic blacklisting entirely
            /*
    ...
    public function is_ip_blocked($ip) {
        ...
        return false; // FIX: Disable all IP blocking
    }
